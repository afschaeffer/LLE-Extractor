...
#              sheet and parses through a Drawing Set PDF, extracting identified
...
# Version 3: Updated - Removed function parsing for the drawing set data due to it increasing runtime in 
#            leiu of manually entering data.
#Version 4: Updated - Added functionality for dynamic cwd idenitfication, allowing the documents to be relocated
#           within the user's personal computer for faster processing time
#Version 5: Updated to pass exceptions due to PDF File Read errors line 194
#----------------------------------------------------------------------------------------------------
...
def get_package_req(pac_sheet, package_type, m_row_min, m_row_max, p_row_min,  p_row_max, e_row_min, e_row_max):
    ...
    def req_drawings_for_package_type(rows, package_type):
        ...
        for num, col in col_names.items():
            ...
            package_name = pac_sheet[f'{col}20'].value
            list_name = package_type + " " + package_name
    ...
    if package_type == "Mechanical":
        req_drawings = req_drawings_for_package_type((m_row_min, m_row_max), package_type)
    # Else if package type = Plumbing
    elif package_type == "Plumbing":
        req_drawings = req_drawings_for_package_type((p_row_min, p_row_max), package_type)
    # Else if package type = Electrical
    elif package_type == "Electrical":
        req_drawings = req_drawings_for_package_type((e_row_min, e_row_max), package_type)
...
def create_files(cwd, pdf, page_nums):
    # Reading file, strict set to false to handle exceptions
...
def clear_sheet():
    extractor_wb = xw.Book('LLE-Extractor-v4.0.xlsm')
...
def run_LLE():
    ...
    # Identifying current directory where program is running from
    cwd = os.path.dirname(os.path.abspath(__file__))
    #Opening Extractor Workbook
    extractor_wb = xw.Book(rf'{cwd}\LLE-Extractor-v4.0.xlsm')
    ...
    cwd = home_sheet.range('B24').value
